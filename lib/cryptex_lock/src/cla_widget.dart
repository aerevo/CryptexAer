...
class _CryptexLockState extends State<CryptexLock> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  final Color _deepOrange = const Color(0xFFFF6F00);  // Primary (Deep Orange 900)
  final Color _pureWhite = const Color(0xFFFFFFFF);   // Success/Text
  final Color _alertRed = const Color(0xFFFF3D00);    // Error (Deep Orange-Red)
  final Color _matteBlack = const Color(0xFF121212);  // Background
  final Color _charcoalGray = const Color(0xFF2E2E2E); // Borders/Panels
...
Widget build(BuildContext context) {
  ...
  // 🔥 FIXED: Use Industrial Colors
  Color activeColor = (state == SecurityState.SOFT_LOCK || state == SecurityState.HARD_LOCK)
      ? _alertRed
      : (state == SecurityState.UNLOCKED ? _pureWhite : _deepOrange);
            ...
            child: Column(
              ...
              children: [
                ...
                AnimatedBuilder(
                  ...
                  builder: (context, child) => Text(status, style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 14, shadows: [Shadow(color: color.withOpacity(0.6 * _pulseController.value), blurRadius: 10)]))
                ),
                const SizedBox(height: 10),
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: const Center(child: Text('FOREN', style: TextStyle(color: Color(0xFF00FF00), fontSize: 7, fontWeight: FontWeight.w900, fontFamily: 'Courier', letterSpacing: 1.5))),
                  ),
                ),
              ],
            ),
...
// 🔥 SUBTLE EMBOSSED NUMBER (Industrial style - reduced glow)
Widget _buildEmbossedNumber(String digit, {required bool isActive, required Color activeColor}) {
  if (isActive) {
    // 🔥 Active State: Subtle orange glow instead of eye-burning cyan
    return ShaderMask(
      ...
      child: Text(
        ...
        style: TextStyle(
          ...
          shadows: [
            ...
            // 🔥 FIXED: Subtle orange glow (not blinding)
            Shadow(offset: Offset.zero, blurRadius: 6, color: activeColor.withOpacity(0.3)),
          ],
        ),
      ),
    );
...
Widget _buildSensorBox(String label, double val, Color color, IconData icon) {
  ...
  Color c = isMatch ? _pureWhite : (val > 0.05 ? _alertRed : const Color(0xFF333333));
  return Container(
    width: 55, height: 42,
...
Widget _buildPatternBox(String label, double score, Color color) {
  ...
  Color c = isMatch ? _pureWhite : (score > 0.1 ? _alertRed : const Color(0xFF333333));
  return Container(
    width: 55, height: 42,
    decoration: BoxDecoration(border: Border.all(color: c, width: 1.5), borderRadius: BorderRadius.circular(8), color: isMatch ? c.withOpacity(0.15) : Colors.transparent),
...
Widget _buildWarningBanner() {
  return Container(
    ...
    decoration: BoxDecoration(color: _alertRed.withOpacity(0.1), border: Border.all(color: _alertRed), borderRadius: BorderRadius.circular(8)),
    child: Row(children: [Icon(Icons.error_outline, color: _alertRed, size: 18), const SizedBox(width: 10), Expanded(child: Text(widget.controller.threatMessage, style: TextStyle(color: _alertRed, fontSize: 10, fontWeight: FontWeight.w900)))])
  );
}
}

// ============================================
// 🔥 MOUNTING BRACKET PAINTER
// ============================================
...
// (Industrial style - subtle glow only)
// ============================================
class MetallicCylinderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 🔥 STEP 4: Subtle Rim Lighting (Industrial - not blinding)
    ...
    // 🔥 STEP 6: Active State - SUBTLE orange glow
    if (isActive) {
      final glowPaint = Paint()
        ..shader = RadialGradient(
          ...
          radius: 0.9,
          colors: [
            activeColor.withOpacity(0.12), // 🔥 REDUCED: Subtle industrial glow
      ...
      final borderPaint = Paint()
        ...
        ..shader = LinearGradient(
          ...
          colors: [
            activeColor.withOpacity(0.5), // 🔥 REDUCED from 0.7
            activeColor.withOpacity(0.25), // 🔥 REDUCED from 0.3
...
// ============================================
// 🔥 END OF FILE - V13.0 COMPLETE
// ============================================
//
// ✅ COLOR SCHEME SUMMARY:
// - Primary: Deep Orange (#FF6F00) - Industrial warning/alert
// - Success: Pure White (#FFFFFF) - Clean & readable  
// - Error: Deep Orange Red (#FF3D00) - Critical alerts
// - Background: Matte Black (#121212) - Premium feel
// - Borders: Charcoal Gray (#2E2E2E) - Subtle structure
//
