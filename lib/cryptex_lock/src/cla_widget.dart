// 🎯 Z-KINETIC WIDGET V3.0 — PRODUCTION READY
// Removed: Fake security theatre
// Added: Real jailbreak detection
...
class _CryptexLockState extends State<CryptexLock> 
  ...
  // Cached sensor data (performance optimization)
  double _cachedAccelX = 0.0;
  double _cachedAccelY = 0.0;
  ...
  /// REAL Jailbreak Detection (not fake method channel)
  ...
  /// OPTIMIZED: Throttled sensor updates (reduce setState calls)
  void _startListening() {
    _accelSub?.cancel();
    
    _accelSub = userAccelerometerEvents.listen((UserAccelerometerEvent e) {
      final rawMag = e.x.abs() + e.y.abs() + e.z.abs();
      
      // Cache sensor data
      _cachedAccelX = e.x;
      _cachedAccelY = e.y;
      
      // Register with controller (always)
  ...
  // Haptic feedback
  void _triggerLightTik() => HapticFeedback.selectionClick();
  void _triggerMediumTik() => HapticFeedback.lightImpact();
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        if (_shakeController.isAnimating) {
          shakeOffset = 10 * (0.5 - (_shakeController.value - 0.5).abs()) * 
                       (widget.controller.state == SecurityState.SOFT_LOCK ? 1 : 0);
        }
        ...
        Row(
          ...
          children: [
            ...
            _buildModernMetric(
              ...
              isGood: widget.controller.motionEntropy > 0.35,
              color: activeColor,
            ),
            Container(width: 1, height: 30, color: Colors.grey[800]),
            _buildModernMetric(
  ...
  Widget _buildThreatAlert() {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            _colFail.withOpacity(0.15),
            _colFail.withOpacity(0.05),
          ],
        ),
        borderRadius: BorderRadius.circular(12),
