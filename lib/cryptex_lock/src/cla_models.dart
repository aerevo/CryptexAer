...
class ClaConfig {
  ...
  const ClaConfig({
    ...
    this.botDetectionSensitivity = 0.6, // 0.6 is balanced for humans
...
class BiometricSignature {
  ...
  double get humanConfidence {
    ...
    // Weight: Magnitude (natural human shake is 0.15-3.0)
    if (averageMagnitude > 0.15 && averageMagnitude < 4.0) score += 0.3;
    
    // Weight: Variance (humans are inconsistent)
    if (frequencyVariance > 0.05) score += 0.25;
    
    // Weight: Entropy (randomness indicator - machines are repetitive)
    if (patternEntropy > 0.4) score += 0.25;
